<template >
  <div>
      <b-navbar class="navbar" type="dark" variant="primary">
          <b-navbar-nav>
              <div class="d-flex flex-row mb-3">
              </div>
              <div class="d-flex flex-.flex-row">


              </div>

              <img class="img1" src="@/assets/img/logocamellapp.png">


              <nav class="navbar bg-danger-fluid">
                  <div class="row">
                      <div class="col-md-4"><button type="button" class="btn1 btn-danger">INICIO</button>
                      </div>

                      <div class="col-md-4 offset-md-4"><button type="button" class="btn2 btn-success">SALIR</button>
                      </div>
                  </div>

                  <form class="container-fluid">
                      <div class="row">
                          <div class="col-md-2">
                              <a class="justify-content-start">
                              </a>

                          </div>
                          <div class="justify-content-end">


                          </div>
                      </div>

                  </form>
              </nav>

          </b-navbar-nav>
      </b-navbar>
      <div>
          <div class="row">
              <div class="col-3">


                  <b-container class="row">

                      <b-row class="my1">
                          <b-col sm="5">
                              <div class="perfil">
                                  <div class="container bg-primary"></div>
                                  <b-row>
                                      <b-col>
                                          <h5>JAVIER GALINDEZ</h5>
                                          <img class="img1"
                                              src="data:image/jpeg;base64,">
                                          <br>
                                          <br>
                                          <button>Actualizar Datos</button>
                                      </b-col>
                                  </b-row>
                              </div>
                              <div class="combo">
                                  <img src="" class="icon">
                                  <div class="preguntas1">

                                      <div class="preguntas">
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">
                                              PERFIL
                                          </b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1  " sm="4" @click="guardar()"
                                              variant="outline-primary">
                                              VER
                                              EMPLEO </b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">VER
                                              USUARIOS
                                              POSTULANTES</b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">VER
                                              USUARIOS
                                              DE EMPLEADOR </b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">VER
                                              CATEGORIAS </b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">VER
                                              DETALLER
                                              DE OFERTA </b-button>
                                          <br>
                                          <br>
                                          <b-button class="button1" @click="guardar()" variant="outline-primary">VER
                                              LOS
                                              ARCHIVOS </b-button>
                                      </div>

                                  </div>
                              </div>
                          </b-col>
                          <b-col sm="6">

                          </b-col>



                      </b-row>
                  </b-container>
              </div>
          
     
          <div class="col-6">
              <div>


                <div>
        <div class="container">
        <div class="row">
  <div>
    <b-card>
    <template #header>
      <b-card-img src="https://www.semana.com/resizer/JmiB52VJxZmk799j7D2CEeTZ1x4=/arc-anglerfish-arc2-prod-semana/public/R52D6MSO7ZB4DF3W4QM4LECYIA.jpg" img-top class="w-50"></b-card-img>
      <br>  <br>
      <h4 class="mb-0 m-1"> <input type="text" class="border-0 bg-opacity-100" placeholder="Ayudante General"></h4>
      <input type="text" placeholder="ej: Se requiere ayudante de construccion por 6 meses" class="border-0 bg-opacity-100">
      <h6></h6>
      
      <div class="row d-flex">
        <div class="col-4"><p class="d-flex justify-content-end">Pago:</p></div>
        <div class="col-4"><input type="text" class="bg-opacity-100 border-0" placeholder="$ 1'000.000"></div>
        <div class="col-4"><p class="d-flex justify-content-top">COP</p></div>
        
      </div>
      </template>
      <b-card-body class="p-0">
        <b-card-title><input type="text" placeholder="Luis alberto posada" class="border-0"></b-card-title>
        <b-card-sub-title class="mb-2 justify-content-center"><input type="text" placeholder="bordo - Cauca" class="border-0"></b-card-sub-title>
<input type="text" class="border-0 align-content-center" placeholder="Nombre de la empresa">
      </b-card-body>
      <b-list-group flush class="b-0"><hr class="m-0">
        <b-list-group-item href="">Detalles de la Oferta</b-list-group-item>

        <b-list-group-item href="#">Postulados</b-list-group-item>
        <b-list-group-item href="#"></b-list-group-item>
      </b-list-group>
  
      <b-card-body class="p-2">
        <h6>Contacto:</h6>
        <a href="#" class="card-link">Facebook</a>
        <a href="#" class="card-link">Whatsapp</a>
      </b-card-body>
  
      <b-card-footer footer-bg-variant="success" footer-text-variant="white">Guardar</b-card-footer>
      <b-card-footer footer-bg-variant="danger" footer-text-variant="white">Eliminar</b-card-footer>
      </b-card>
  </div>
</div>
    </div>
    </div>


              </div>


          </div>
      </div>
      </div>

  </div>




</template>
<script>

export default {
  data() {
      return {
          form: {


          },

      }
  },

}
</script>

<style>
.preguntas {

  text-align: center;


}

.button1 {

  width: 300px;
  height: 40px;
  text-align: center;
}

.img1 {
  border-radius: 90%;
  border: 1px solid;
  width: 90px;
  height: 90px;
}

.img3 {
  margin-left: 1140px;
}

.camellapp {
  background-color: white;
  border: 1px solid;
  width: 800px;
  height: 50px;
}

.tarjeta1 {
  background-color: white;
  border: 1px solid;
  width: 800px;
  height: 11em;
}

.img5 {
  border-radius: 90%;
  border: 1px solid;
  width: 75px;
  height: 75px;
  margin-top: 18%;

}

.row {
  padding: 1px;
}

.problemas {
  text-align: left;
}


.perfil {
  border: 1px solid;
  width: 27em;
  height: 200px;
  background-color: white;
  border-radius: 10px 10px 10px 10px;
  padding: 5px;
  border-radius: 5px;
}

.preguntas1 {
  width: 25em;
  height: 30em;

  border-radius: 5px;
}

.Combo {
  width: 50em;
  border: 1px solid;
  height: 46.2em;
}
</style> 